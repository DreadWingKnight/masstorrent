...
#End Region
    ...
    Private Sub ProcessNow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ProcessNow.Click
        Dim ScrapeData As New TorrentDictionary
        torrentfileload = FreeFile()
        FileOpen(torrentfileload, "C:\Documents and Settings\Administrator\My Documents\page\scrape.torrent", OpenMode.Binary, OpenAccess.Default)
        Dim intermediaryscrapedata As String = Space(FileLen("C:\Documents and Settings\Administrator\My Documents\page\scrape.torrent"))
        FileGet(torrentfileload, intermediaryscrapedata)
        FileClose(torrentfileload)
        ScrapeData.Parse(intermediaryscrapedata)
        Dim ScrapeFiles As New TorrentDictionary
        ScrapeFiles = ScrapeData.Value("files")
        ...
        For Each TorrentFile As String In FolderFiles
            ...
            Dim HashToCalculate() As Byte
            HashToCalculate = System.Text.Encoding.Default.GetBytes(TorrentInfo.Bencoded)
            sha1.ComputeHash(HashToCalculate)
            Dim HashConvert As New EAD.Conversion.HashChanger
            HashConvert.bytehash = sha1.Hash
            Dim FileScrape As New TorrentDictionary
            FileScrape = ScrapeFiles.Value(HashConvert.rawhash)
            Dim Seeds As New TorrentNumber
            Dim Downloaders As New TorrentNumber
            Seeds = FileScrape.Value("complete")
            Downloaders = FileScrape.Value("incomplete")
            If Seeds.Value = 0 And Downloaders.Value = 0 Then
                Kill(TorrentFile)
        ...
        Next
        MsgBox("Files Deleted: " & ProcessedFiles)
